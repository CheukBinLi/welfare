...
public class Signature {
	...
	public static boolean checkIsSignValidFromResponseString(String responseString) throws ParserConfigurationException, IOException, SAXException {
		...
		String signFromAPIResponse = map.get("job").toString();
		if (signFromAPIResponse == "" || signFromAPIResponse == null) {
			logger.warn("API返回的数据签名数据不存在，有可能被第三方篡改!!!");
			return false;
		}
		logger.info("服务器返回的签名是:" + signFromAPIResponse);
		// 清掉返回数据对象里面的Sign数据（不能把这个数据也加进去进行签名），然后用签名算法进行签名
		map.put("job", "");
		// 将API返回的数据根据用签名算法进行计算新的签名，用来跟API返回的签名进行比较
	...
	public static String getSign(Map<String, Object> map, String apiKey) {
		if (StringUtils.isBlank(apiKey)) {
			logger.error("请传入apiKey");
		}

		ArrayList<String> list = new ArrayList<String>();
		for (Map.Entry<String, Object> entry : map.entrySet()) {
			if (entry.getValue() != null && entry.getValue() != "" && !entry.getKey().equals("apiKey")) {
				list.add(entry.getKey() + "=" + entry.getValue() + "&");
			}
		}
		int size = list.size();
	...
	public static boolean checkIsSignValidFromResponseString(String responseString, String apiKey) throws ParserConfigurationException, IOException, SAXException {
		Map<String, Object> map = XMLParser.getMapFromXML(responseString);
		return checkIsSignValidFromResponseMap(map, apiKey);
	}
	 ...
	 *
	 * @param map
	 *            API返回的XML数据字符串转换成map
	 * @return API签名是否合法
	 * @throws ParserConfigurationException
	 * @throws IOException
	 * @throws SAXException
	 */
	public static boolean checkIsSignValidFromResponseMap(Map<String, Object> map, String apiKey) throws ParserConfigurationException, IOException, SAXException {
		Util.log(map.toString());

		String signFromAPIResponse = map.containsKey("job") ? map.get("job").toString() : null;
		if (signFromAPIResponse == "" || signFromAPIResponse == null) {
			logger.warn("API返回的数据签名数据不存在，有可能被第三方篡改!!!");
			return false;
		}
